...
import { Phone, ChevronDown, Star, Clock, MapPin, ArrowRight, Bone, Activity, Brain, Dumbbell, Zap, HeartPulse, Home, CheckCircle2 } from 'lucide-react'
...
export default function HomeClient() {
  return (
    <main>
      {/* ════ HERO ════ */}
      <section id="hero" className="relative min-h-[100svh] flex flex-col justify-center overflow-hidden" aria-label="Hero">
        {/* Three.js background */}
        <HeroScene />

        {/* Gradient overlays */}
        <div className="absolute inset-0 bg-gradient-to-br from-ink-950/90 via-ink-900/80 to-ink-900/70 pointer-events-none" />
        <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 70% 50% at 50% 40%, rgba(76,175,80,0.07), transparent 65%)' }} />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 pt-24 sm:pt-28 pb-10 w-full">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            ...
            <div className="text-center lg:text-left">
              ...
              <motion.h1 initial={{ opacity:0, y:26 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.75, delay:0.1 }}
                ...
                <br />
                <span className="text-white/85 text-2xl sm:text-3xl md:text-4xl font-semibold italic">Reclaim Your Life.</span>
              </motion.h1>

              <motion.p initial={{ opacity:0, y:18 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.7, delay:0.22 }}
                className="text-gray-400 font-sans text-base sm:text-lg leading-relaxed mb-8 max-w-lg mx-auto lg:mx-0">
                Expert physiotherapy for spinal disorders, sports injuries, neurological conditions and chronic pain — personalized to you.
              </motion.p>

              <motion.div initial={{ opacity:0, y:14 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.7, delay:0.32 }}
              ...
              <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.6 }}
                ...
                <div className="flex -space-x-2">
                  {['#C9A84C','#4CAF50','#A8872F','#388E3C','#E8C96A'].map((c, i) => (
                    <div key={i} className="w-7 h-7 sm:w-8 sm:h-8 rounded-full border-2 border-ink-900 flex items-center justify-center text-[9px] font-bold text-ink-900"
                      style={{ background: c }}>
                      {['R','A','S','M','P'][i]}
                    </div>
                  ))}
        ...
        <motion.button
          ...
          className="absolute bottom-6 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-1 text-gray-500 hover:text-brand-gold transition-colors"
      ...
      <section id="services-preview" className="py-16 md:py-24 relative" aria-labelledby="services-heading">
        <div className="absolute inset-0" style={{ background:'radial-gradient(ellipse 70% 50% at 80% 50%,rgba(76,175,80,.05),transparent 60%)' }} />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
          <Anim className="text-center mb-10 md:mb-14">
            <div className="section-badge mx-auto w-fit">Our Specialties</div>
            <h2 id="services-heading" className="font-display text-3xl sm:text-4xl md:text-5xl font-bold mb-3">
              <span className="text-white">What We </span><span className="text-brand-green">Treat</span>
            </h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-5 mb-10">
            {SERVICES.map(({ icon: Icon, title, color, href }, i) => (
              <Anim key={title} direction="up" delay={i * 0.07}>
                <Link href={href}
                  className="glass rounded-xl sm:rounded-2xl p-4 sm:p-6 text-center border border-white/5 hover:border-brand-gold/25 transition-all duration-300 group block h-full">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl mx-auto mb-2.5 sm:mb-3 flex items-center justify-center transition-transform duration-300 group-hover:scale-110"
                    style={{ background:`rgba(${color==='#C9A84C'?'201,168,76':'76,175,80'},.12)` }}>
                    <Icon size={18} style={{ color }} />
                  </div>
                  <p className="font-sans font-semibold text-xs sm:text-sm text-white group-hover:text-brand-gold transition-colors">{title}</p>
                </Link>
              </Anim>
            ))}
          </div>
          ...
          <Anim className="text-center">
            <Link href="/services" className="btn-outline w-full sm:w-auto inline-flex">
      ...
      <section className="py-16 md:py-24 relative" aria-labelledby="why-heading">
        <div className="absolute inset-0" style={{ background:'radial-gradient(ellipse 60% 50% at 20% 50%,rgba(201,168,76,.04),transparent 60%)' }} />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            <div>
              <Anim>
                <div className="section-badge w-fit mb-3">Why Choose Us</div>
                <h2 id="why-heading" className="font-display text-3xl sm:text-4xl md:text-5xl font-bold leading-tight mb-4 sm:mb-6">
                ...
                <Link href="/about" className="btn-brand shadow-gold w-full sm:w-auto inline-flex">
            ...
            <Anim direction="right">
              <div className="relative max-w-xs sm:max-w-sm mx-auto lg:max-w-none">
                <div className="glass-gold rounded-2xl sm:rounded-3xl p-8 sm:p-10 text-center relative overflow-hidden">
                  ...
                  <div className="relative w-20 sm:w-24 mx-auto mb-5">
                    {Array.from({ length:14 }).map((_,i) => (
                      <motion.div key={i} initial={{ scaleX:0 }} whileInView={{ scaleX:1 }} viewport={{ once:true }}
                        ...
                        style={{
                          ...
                          background:i%2===0?'linear-gradient(90deg,rgba(201,168,76,.75),rgba(201,168,76,.3))':'linear-gradient(90deg,rgba(76,175,80,.55),rgba(76,175,80,.25))',
                        }}
                      />
                    ))}
                  </div>
                ...
                <motion.div animate={{ y:[-4,5,-4] }} transition={{ duration:3, repeat:Infinity, ease:'easeInOut' }}
                ...
                <motion.div animate={{ y:[5,-4,5] }} transition={{ duration:3.5, repeat:Infinity, ease:'easeInOut' }}
          ...
          <Anim direction="scale">
            <div className="glass-gold rounded-2xl sm:rounded-3xl p-7 sm:p-10 md:p-14 text-center relative overflow-hidden border border-brand-gold/15">
              ...
              <div className="relative z-10">
                <h2 id="cta-heading" className="font-display text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-3">
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                  <a href="tel:+918128370332" className="btn-brand shadow-gold w-full sm:w-auto">
                    <Phone size={15} /> Call: +91 81283 70332
                  </a>
                  <Link href="/contact" className="btn-outline w-full sm:w-auto">Book Online</Link>
                </div>
